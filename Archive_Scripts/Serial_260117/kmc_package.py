#H5PY used inside the Array Class - Until 26/01/2017
...
class Array:
	...
	def __init__(self, input_list, input_prob, f):
		if type(input_list) == list:
			...
			self.object_constructor(input_list, input_prob, f)
	...
	def standard_constructor(self, input_list, input_prob, f):
		...
		self.input_timeLimit = input_list[9] #Time limit (s) for each run
		self.input_images = input_list[10] #Number of images to be saved
	...
	def save_evolution(self):
		...
		dset_evo.attrs['timeLimit'] = self.input_timeLimit
		...
		dset_doubleFreq_name = 'run{:d}/f_double'.format(self.input_run)
		dset_doubleFreq = self.output_file.create_dataset(dset_doubleFreq_name, data=self.freq_double)
		dset_doubleFreq.attrs['input_double'] = self.input_double
		dset_doubleFreq.attrs['attempt_freq'] = self.input_attemptFreq
		
		dset_singleFreq_name = 'run{:d}/f_single'.format(self.input_run)
		dset_singleFreq = self.output_file.create_dataset(dset_singleFreq_name, data=self.freq_single)
		dset_singleFreq.attrs['input_single'] = self.input_single
		dset_singleFreq.attrs['attempt_freq'] = self.input_attemptFreq
